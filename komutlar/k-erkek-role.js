...
if(!role) return message.channel.send(nn.setTitle('Bir hata oldu!')
.setColor('#f583fa')
.setDescription(`Bir rol etiketlemeyi unuttunuz.`))
.then(a => a.delete({timeout: 10000}));
data.set(`erkek.${message.guild.id}`, role.id);
message.channel.send(nn.setTitle(`İşte bu kadar!`)
.setDescription(`Kayıt için kullanılacak: ${role} rol olarak seçtiniz.`))
.then(a => a.delete({timeout: 10000}));

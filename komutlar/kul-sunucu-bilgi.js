...
exports.run = async (client, message, args) => {
  if (!message.member.hasPermission("MANAGE_GUILD")) {
    const embed = new Discord.RichEmbed()
      ...
      .setColor("RED")
      .setDescription(
        `:x: Bu komutu kullanabilmek için \`Sunucuyu Yönet\` yetkisi gerekmektedir!`
      )
    ...
    var embed = new Discord.RichEmbed()
      ...
      .addField(
        "Roller",
        `**Rol Sayısı: ** ${(message.guild.roles.map(a => a.name),
        message.guild.roles.size)}`
      )
      .addField(
        "Kanallar",
        `**Kanal Sayısı: ** ${(message.guild.channels.map(a => a.name),
